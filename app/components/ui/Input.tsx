...
const field =
  "w-full rounded-xl border border-brand-muted/50 bg-brand-bg/90 px-4 py-3 text-sm text-brand-cream placeholder:text-brand-muted/45 transition focus:border-brand-lime focus:outline-none focus:ring-1 focus:ring-brand-lime";
...
export function Input({ label, hint, error, id, className = "", ...rest }: Props) {
  return (
    <div className="space-y-1.5">
      ...
      />
      {hint && !error ? (
        <p id={`${id}-hint`} className="text-xs text-brand-muted/80">
          {hint}
        </p>
      ) : null}
